...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y)
{
  ...
  if (c >= 'A' && c <= 'Z')
  {
    // Para letras maiúsculas: bloco "nada" (1) + dígitos (10)
    index = (c - 'A' + 11) * 8;
  }
  else if (c >= '0' && c <= '9')
  {
    // Para dígitos: bloco "nada" conta como 1
    index = (c - '0' + 1) * 8;
  }
  else if (c >= 'a' && c <= 'z')
  {
    // Para minúsculas: após o bloco "nada", 10 dígitos e 26 letras maiúsculas (1+10+26 = 37)
    index = (c - 'a' + 37) * 8;
  }
  else
  {
    // Se o caractere não for reconhecido, usa o bloco "nada"
    index = 0;
  }
  ...
  for (uint8_t i = 0; i < 8; ++i)
  {
    ...
    for (uint8_t j = 0; j < 8; ++j)
    {
      // Testa se o bit j está ativo (retorna true se estiver)
      ssd1306_pixel(ssd, x + i, y + j, (line & (1 << j)) != 0);
    }
  }
}
